...
class FeatureFactory:
    ...
    def computeFeatures(self, words, previousLabel, position):
        ...
        features.append("word-1=" + words[position-1])
        features.append("word-2=" + words[position-2])
        if position +1 < len(words) :
          features.append("word+1=" + words[position+1])
          if words[position+1][0].isupper and currentWord[0].isupper:
            features.append("prevcap=")
        if position +2 < len(words) :
         features.append("word+2=" + words[position+2])       
        if currentWord[0].isupper() and len(currentWord) > 1:
          if currentWord[1].islower():            
            features.append("case=Title")       
        if words[position-1][0].isupper and currentWord[0].isupper:
          features.append("nextcap=")
        if words[position-1][0].islower and currentWord[0].isupper:
          features.append("nextcap2=")
        #if words[position -1] != '.' and currentWord[0].isupper() :
        #  features.append("dot")
